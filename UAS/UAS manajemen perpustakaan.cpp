...
void sortBuku() {
    ...
    printf("Buku berhasil diurutkan berdasarkan Abjad.\n");
}
...
void hapusBuku() {
    ...
    scanf("%d", &id); // Membaca ID buku yang ingin dihapus
    ...
    while (temp) {
        if (temp->id == id) {
            if (prev) {
                prev->next = temp->next; // Menghapus buku dengan menghubungkan prev ke buku berikutnya
            } else {
                head = temp->next; // Menghapus buku pertama
            }
            free(temp); // Menghapus memori buku
...
void editBuku() {
    ...
    scanf("%d", &id); // Membaca ID buku yang ingin diedit

    Buku *temp = head;
    while (temp) {
        if (temp->id == id) {
            printf("Masukkan judul baru: ");
            scanf(" %[^\n]%*c", temp->judul); // Membaca judul baru
            printf("Masukkan penulis baru: ");
            scanf(" %[^\n]%*c", temp->penulis); // Membaca penulis baru
...
int main() {
    ...
    while (1) {
        ...
        printf("6. Urutkan Buku Berdasarkan Abjad\n");
        printf("7. Hapus Buku\n");
        printf("8. Edit Buku\n");
        printf("9. Keluar\n");
